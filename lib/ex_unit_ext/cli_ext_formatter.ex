defmodule ExUnitExt.CLIExtFormatter do
  @moduledoc false
  use GenServer
  ...
  defp print_start(config, opts) do
    message =
      whole_line(
    ...
    puts([:reverse, :blank, message, :blank], config)
  end

  defp whole_line(message, config, chars) do
    width = config.width + chars
    String.pad_trailing(message, width)
  end
  ...
  def handle_cast({:suite_finished, times_us}, config) do
    ...
    for %{state: {:failed, failures}} = test <- config.failures do
      formatted =
        format_test_failure(
          test,
          failures,
  ...
  def handle_cast({:test_started, %ExUnit.Test{} = test}, config) do
    config = if config.trace do
      IO.write("#{trace_test_started(test)} [#{trace_test_line(test)}]")
      config
    else
      print_sign(:pending, config)
      %{config | pending_counter: config.pending_counter + 1}
    end
  ...
  def handle_cast({:test_finished, %ExUnit.Test{state: nil} = test}, config) do
    pending = config.pending_counter

    if config.trace do
      IO.puts(success(trace_test_result(test), config))
    else
      print_success_sign(pending, config)
    end
  ...
  defp print_sign(name, config) do
    write([:success, sign(name, config)], config)
  end

  defp print_success_sign(pending, config) do
    backspaces = List.duplicate("\b", pending)

    pending_signs =
      if pending > 1 do
        List.duplicate(sign(:pending, config), pending - 1)
      else
        []
      end

    write([backspaces, :success, sign(:success, config), :pending, pending_signs], config)
    # IO.write("\b")
  ...
  defp sign(name, config) do
    ...
    if is_list(sign), do: Enum.random(sign), else: sign
  end

  defp write(ansi, config) do
    Escape.write(ansi, theme: config.theme, emit: config.theme.enabled)
  end

  defp puts(ansi, config) do
    Escape.puts(ansi, theme: config.theme, emit: config.theme.enabled)
  end
  ...
  defp print_summary(config, force_failures?) do
    ...
    cond do
      ...
      true ->
        puts([:success, :reverse, :blank, message, :blank], config)
